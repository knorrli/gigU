...
class Event < ApplicationRecord
  ...
  def self.filter(filter_hash)
    ...
    if profile_id = filter_hash.try(:[], :i)
      events = events.merge(
        joins(event_interests: :profile).where(event_interests: { profile_id: profile_id})
      )
    end
    ...
    if profile_id = filter_hash.try(:[], :c)
      ...
      events = events.merge(
        includes(location: [:city, :region]).where(
    ...
    if location_ids = filter_hash.try(:[], :l_ids)
      events = events.merge(
        where.not(location_id: location_ids)
      )
    end
    if city_ids = filter_hash.try(:[], :c_ids)
      events = events.merge(
        where.not(locations: { city_id: city_ids })
      )
    end
    if region_ids = filter_hash.try(:[], :r_ids)
      events = events.merge(
        where.not(locations: { region_id: region_ids })
      )
    end
    events
  end
